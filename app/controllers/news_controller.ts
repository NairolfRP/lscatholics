...
import { createNewsSearchParamsValidator } from '#validators/news'
...
export default class NewsController {
  async index({ request, inertia }: HttpContext) {
    ...
    try {
      ...
      return inertia.render('news/all', {
        articles: NewsTransformer.paginate(data.all(), data.getMeta()).useVariant('publicList'),
  ...
  async single({ inertia, params }: HttpContext) {
    ...
    return inertia.render('news/single', {
      post: NewsTransformer.transform(post).useVariant('publicDetails'),
    })
  }
}
