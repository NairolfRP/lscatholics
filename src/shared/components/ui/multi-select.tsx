...
export function MultiSelect({
  ...
  function toggleValue(value: string | null) {
    const getNewSet = (prev: Set<string | null>) => {
      ...
      if (newSet.has(value)) {
        // oxlint-disable-next-line drizzle/enforce-delete-with-where
        newSet.delete(value)
      } else {
        newSet.add(value)
      }
...
export function MultiSelectValue({
  ...
  const checkOverflow = useCallback(() => {
    ...
    const dataSelectedItems = containerElement.querySelectorAll<HTMLElement>('[data-selected-item]')

    if (overflowElement != null) overflowElement.style.display = 'none'
    dataSelectedItems.forEach((child) => child.style.removeProperty('display'))
    let amount = 0
    for (let i = dataSelectedItems.length - 1; i >= 0; i--) {
      const child = dataSelectedItems[i]
      if (containerElement.scrollWidth <= containerElement.clientWidth) {
        break
      }
      amount = dataSelectedItems.length - i
