...
const donateBaseSchema = z.object({
  amount: z
    .int({
      error: (issue) =>
...
const organizationSchema = z.discriminatedUnion('isOrganization', [
  z.object({
    isOrganization: z.literal(true),
    organizationName: z.string().trim().min(1, { error: "Le nom de l'organisation est requis." }),
  }),
  z.object({
...
export const donateSchema = donateBaseSchema.and(organizationSchema)
