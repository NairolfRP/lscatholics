<template>
  <div
    class="relative overflow-hidden rounded-lg border bg-linear-to-r from-[#5865F2]/5 to-background"
  >
    <div class="absolute inset-0 bg-[#5865F2]/5 opacity-50"></div>

    <div class="relative p-6">
      <div class="flex flex-col sm:flex-row gap-4 items-center justify-between">
        <div class="flex items-center gap-4">
          <div class="w-12 h-12 rounded-xl bg-[#5865F2] flex items-center justify-center shadow-md">
            <svg
          ...
          <div>
            ...
            <p class="text-sm text-muted-foreground">
              {{ discordUser ? 'Compte lié' : 'Liez votre compte Discord' }}
            </p>
          </div>
        </div>

        <div v-if="discordUser" class="flex items-center gap-4">
          <div class="flex items-center gap-3">
            <Avatar class="relative size-10">
              <AvatarImage
                v-if="discordUser?.avatar"
                :src="discordUser.avatar"
                :alt="discordUser?.username"
              />
              <AvatarFallback
                class="bg-linear-to-br from-[#5865F2] to-[#4752C4] text-white font-semibold"
              >
                {{ discordUser?.username?.charAt(0).toUpperCase() }}
              </AvatarFallback>
            </Avatar>
            <span class="font-medium">{{ discordUser?.username }}</span>
          </div>
          <Dialog>
            <DialogTrigger>
              <Button
            ...
            <DialogContent>
              ...
              <DialogFooter>
                <DialogClose as-child>
                  <Button type="button" @click="unlinkDiscord" variant="destructive" class="w-full">
                    Délier le compte
                  </Button>
                </DialogClose>
              </DialogFooter>
...
const props = usePageProps<{ discordUser?: { id: number; username: string; avatar?: string } }>()
